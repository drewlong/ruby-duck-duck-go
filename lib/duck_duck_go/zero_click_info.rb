module DuckDuckGo
  ...
  class DuckDuckGo::ZeroClickInfo
    ...
    def self.by(result)
      ...
      abstract_url = URI.parse(CGI.escape(result['AbstractURL'])) unless result['AbstractURL'].empty?
      image = URI.parse(CGI.escape(result['Image'])) unless result['Image'].empty?
      ...
      definition_url = URI.parse(CGI.escape(result['DefinitionURL'])) unless result['DefinitionURL'].empty?
